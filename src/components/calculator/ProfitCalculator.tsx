  ...
  return (
    <CalculatorLayout>
      <CalculatorCard title="단가 및 수량 입력" description="매수하신 가격과 현재가, 보유 수량을 입력하세요.">
        <InputField id="buyPrice" label="매수가 (원)" type="number" placeholder="예: 50000" unit="원" value={buyPrice} onChange={e => setBuyPrice(e.target.value)} />
        <InputField id="currentPrice" label="현재가 (원)" type="number" placeholder="예: 55000" unit="원" value={currentPrice} onChange={e => setCurrentPrice(e.target.value)} />
        <InputField id="quantity" label="수량 (주)" type="number" placeholder="예: 100" unit="주" value={quantity} onChange={e => setQuantity(e.target.value)} />
      </CalculatorCard>

      <ResultCard title="수익률 계산 결과" emptyMessage="매수가와 현재가를 입력하시면\n수익률이 자동으로 계산됩니다." isValid={result.valid}>
        <ResultHighlight
          label="예상 수익률"
