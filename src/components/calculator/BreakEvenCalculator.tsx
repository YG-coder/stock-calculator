...
export default function BreakEvenCalculator() {
  ...
  const result = useMemo(() => {
    ...
    if (loss <= 0 || loss >= 100) {
      return { valid: false, requiredRate: 0 };
    }
      ...
      <ResultCard title="본전 회복 계산 결과" emptyMessage="현재 손실률을 입력하시면\n본전을 찾기 위해 필요한 상승률이 계산됩니다." isValid={result.valid}>
        ...
        <div className="rounded-xl bg-slate-50 border border-slate-200 p-5 shadow-sm">
          <p className="text-sm font-medium text-slate-700 leading-relaxed">
            현재 <strong className="text-blue-600">-{result.loss}%</strong> 손실 상태에서 원금을 회복하려면, 남은 자금으로 무려 <strong className="text-red-600">+{result.requiredRate.toFixed(2)}%</strong>의 수익을 내야 합니다.
          </p>
        </div>
      </ResultCard>
    </CalculatorLayout>
