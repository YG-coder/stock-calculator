    ...
    <CalculatorLayout>
      <div className="space-y-6">
        <CalculatorCard title="1. 자금 및 리스크 정보" description="총 자본금과 1회 매매 허용 리스크를 입력하세요.">
          <InputField id="totalCapital" label="총 투자 자본금" type="number" placeholder="예: 10000000" unit="원" value={totalCapital} onChange={e => setTotalCapital(e.target.value)} />
          <InputField id="riskPercent" label="1회 허용 리스크 (1~2% 권장)" type="number" placeholder="예: 2" unit="%" value={riskPercent} onChange={e => setRiskPercent(e.target.value)} />
        </CalculatorCard>

        <CalculatorCard title="2. 매매 셋업" description="진입 가격과 손절 라인을 입력하세요.">
          <InputField id="entryPrice" label="진입 가격" type="number" placeholder="예: 50000" unit="원" value={entryPrice} onChange={e => setEntryPrice(e.target.value)} />
          <InputField id="stopLossPrice" label="손절 라인" type="number" placeholder="예: 48000" unit="원" value={stopLossPrice} onChange={e => setStopLossPrice(e.target.value)} />
        </CalculatorCard>
      </div>
