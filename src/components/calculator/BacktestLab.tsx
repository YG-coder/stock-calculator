...
import { CalculatorCard, CalculatorLayout, InputField, ResultCard, ResultDetail, ResultHighlight, TextareaField } from "@/components/ui/Shared";
...
export default function BacktestLab() {
  ...
  return <CalculatorLayout>
    <CalculatorCard title="월별 수익률 자료" description="시간순 월 수익률을 % 단위로 붙여넣으세요. 기본 숫자는 사용법 확인용 가상 예시이며 실제 시장 자료가 아닙니다.">
      <TextareaField id="lab-returns" label="월별 수익률(%, 쉼표·공백·줄바꿈 구분)" value={returnsText} onChange={(event) => setReturnsText(event.target.value)} placeholder="예: 2.1, -1.2, 3.4, 0.8" />
