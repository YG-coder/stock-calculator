...
export default function TargetPriceCalculator() {
  ...
  return (
    <CalculatorLayout>
      <CalculatorCard title="목표가 입력" description="매수하신 가격과 원하는 목표 수익률을 입력하세요.">
        ...
        <InputField id="targetRate" label="목표 수익률 (%)" type="number" placeholder="예: 10" unit="%" value={targetRate} onChange={e => setTargetRate(e.target.value)} />
      </CalculatorCard>

      <ResultCard title="목표가 계산 결과" emptyMessage="매수가와 목표 수익률을 입력하시면\n목표 매도가가 자동으로 계산됩니다." isValid={result.valid}>
        <ResultHighlight
          label="목표 매도 가격"
