  ...
  return (
    <CalculatorLayout>
      <CalculatorCard title="가격 정보 입력" description="진입가와 예상 목표가, 손절가를 입력하세요.">
        <InputField id="entryPrice" label="진입 가격 (매수가)" placeholder="예: 50000" unit="원" value={entryPrice} onChange={e => setEntryPrice(e.target.value)} />
        <InputField id="targetPrice" label="목표 가격 (익절가)" placeholder="예: 55000" unit="원" value={targetPrice} onChange={e => setTargetPrice(e.target.value)} />
        <InputField id="stopLossPrice" label="손절 가격 (손절가)" placeholder="예: 48000" unit="원" value={stopLossPrice} onChange={e => setStopLossPrice(e.target.value)} />
      </CalculatorCard>
