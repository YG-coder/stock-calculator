  ...
  return <CalculatorLayout>
    <CalculatorCard title="자산별 가정" description="직접 입력한 기대수익률·변동성·상관계수로 계산합니다. 기본값은 예시일 뿐 시장 전망이나 추천값이 아닙니다.">
      {assets.map((asset, index) => <div key={index} className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <InputField id={`portfolio-weight-${index}`} label="비중" type="number" value={asset.weight} onChange={(event) => updateAsset(index, "weight", event.target.value)} unit="%" />
        <InputField id={`portfolio-return-${index}`} label="연 기대수익률" type="number" value={asset.expectedReturn} onChange={(event) => updateAsset(index, "expectedReturn", event.target.value)} unit="%" />
