...
import { AdvancedPanel, CalculatorCard, CalculatorLayout, GettingStarted, InputEffectNote, InputField, PercentileNote, ResultCard, ResultDetail, ResultHighlight, ResultNote, simulationIssueText } from "@/components/ui/Shared";
...
const initialFields: Fields = { seed: "", paths: "10000", years: "", initialAssets: "", monthlyWithdrawal: "", expectedReturn: "", volatility: "", inflation: "" };
...
export default function SorrSimulationCalculator() {
  ...
  return <CalculatorLayout>
    ...
    <CalculatorCard title="월 인출 생존확률 조건" description="월초 생활비 인출과 월별 수익률 경로를 확률 시뮬레이션으로 계산합니다. 아래 정순·역순 비교와 별개의 분석입니다.">
      ...
      <AdvancedPanel>
        <div className="grid gap-4 sm:grid-cols-2"><InputField id="sorr-mc-paths" label="가상 미래를 몇 번 만들지 (경로 수)" type="number" value={fields.paths} onChange={(event) => set("paths", event.target.value)} unit="개" placeholder="예: 10000" hint="서로 다른 미래를 몇 개 만들어볼지 정합니다." help="이 계산기는 미래를 하나만 예측하지 않고 서로 다른 미래를 수천~수만 개 만들어 결과가 어떻게 퍼지는지를 봅니다. 숫자를 늘리면 결과가 더 안정적이지만 계산이 오래 걸립니다." /><InputField id="sorr-mc-seed" label="결과 재현 번호" type="number" value={fields.seed} onChange={(event) => set("seed", event.target.value)} placeholder="예: 20260824" grouping={false} hint="같은 번호를 넣으면 언제 계산해도 똑같은 결과가 나옵니다." help="결과를 캡처해두거나 남에게 보여줄 때 이 번호가 같으면 상대도 똑같은 화면을 볼 수 있습니다. 신경 쓰지 않아도 되는 값입니다." /></div>
      </AdvancedPanel>
      <InputEffectNote />
      </CalculatorCard>
