...
export default function SorrSimulationCalculator() {
  ...
  return <CalculatorLayout>
    <CalculatorCard title="월 인출 생존확률 조건" description="월초 생활비 인출과 월별 수익률 경로를 확률 시뮬레이션으로 계산합니다. 아래 정순·역순 비교와 별개의 분석입니다.">
      <div className="grid gap-4 sm:grid-cols-2"><InputField id="sorr-mc-assets" label="은퇴 시작 자산" type="number" value={fields.initialAssets} onChange={(event) => set("initialAssets", event.target.value)} unit="원" placeholder="예: 1000000000" /><InputField id="sorr-mc-withdrawal" label="현재 기준 월 생활비" type="number" value={fields.monthlyWithdrawal} onChange={(event) => set("monthlyWithdrawal", event.target.value)} unit="원" placeholder="예: 3000000" /><InputField id="sorr-mc-years" label="인출 기간" type="number" value={fields.years} onChange={(event) => set("years", event.target.value)} unit="년" placeholder="예: 30" /><InputField id="sorr-mc-return" label="연 기대수익률(CAGR)" type="number" value={fields.expectedReturn} onChange={(event) => set("expectedReturn", event.target.value)} unit="%" placeholder="예: 7" /><InputField id="sorr-mc-volatility" label="연 변동성" type="number" value={fields.volatility} onChange={(event) => set("volatility", event.target.value)} unit="%" placeholder="예: 15" /><InputField id="sorr-mc-inflation" label="연 물가상승률" type="number" value={fields.inflation} onChange={(event) => set("inflation", event.target.value)} unit="%" placeholder="예: 2" /><InputField id="sorr-mc-paths" label="경로 수" type="number" value={fields.paths} onChange={(event) => set("paths", event.target.value)} unit="개" placeholder="예: 10000" /><InputField id="sorr-mc-seed" label="재현 시드" type="number" value={fields.seed} onChange={(event) => set("seed", event.target.value)} placeholder="예: 20260824" grouping={false} /></div>
    ...
    {result ? <><CalculatorCard title="월 인출 시뮬레이션 결과" description={`${result.meta.pathsRun.toLocaleString("ko-KR")}개 경로 · ${result.input.months / 12}년`}><ResultHighlight label="기간 종료까지 자산이 소진되지 않은 경로" value={percent(survival)} tone={survival >= 0.8 ? "positive" : "negative"} /><div className="grid gap-4 sm:grid-cols-2"><ResultDetail label="최종 자산 중앙값(현재 가치)" value={won(result.terminal.p50)} unit="원" /><ResultDetail label="최종 자산 P10(현재 가치)" value={won(result.terminal.p10)} unit="원" /><ResultDetail label="소진 경로 비율" value={percent(result.depletion?.rate ?? 0)} /><ResultDetail label="소진 경로 중앙 소진 시점" value={result.depletion ? `${(result.depletion.percentiles.p50 / 12).toFixed(1)}년` : "기간 내 미소진"} /></div></CalculatorCard><CalculatorCard title="가정과 주의사항"><ul className="list-disc space-y-2 pl-5 text-sm text-slate-600"><li>생활비는 매월 초 인출하고 매년 물가상승률만큼 증가합니다.</li>{result.meta.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul></CalculatorCard></> : <ResultCard title="월 인출 시뮬레이션 결과" isValid={false} emptyMessage="값을 입력하고 시뮬레이션을 실행하면 결과가 표시됩니다." />}
  </CalculatorLayout>;
}
