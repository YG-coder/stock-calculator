...
/* 화면 문구의 세율·공제액은 taxRates.ts 를 단일 소스로 삼는다.
   예시 금액도 같은 값에서 계산해, 기준이 바뀌면 예시가 따로 낡지 않게 한다. */
const RATE = OVERSEAS_STOCK_TAX.rateDisplay;
const DEDUCTION = OVERSEAS_STOCK_TAX.basicDeductionDisplay;
const won = (v: number) => new Intl.NumberFormat("ko-KR").format(Math.round(v));
const manwon = (v: number) => `${won(v / 10_000)}만 원`;

function example(gain: number) {
    const taxable = Math.max(0, gain - OVERSEAS_STOCK_TAX.basicDeduction);
    const tax = taxable * OVERSEAS_STOCK_TAX.rate;
    return { gain, taxable, tax, net: gain - tax };
}
const EX_A = example(5_000_000);
const EX_B = example(6_000_000);

export const metadata: Metadata = withPageMetadata(config.metadata, "/overseas-stock-tax-calculator");

const FAQ_ITEMS = [
  { question: "해외주식 양도소득세는 얼마인가요?", answer: `기본공제 ${DEDUCTION}을 초과한 양도차익에 대해 ${RATE}의 세율이 적용됩니다. 예를 들어 수익이 ${manwon(EX_A.gain)}이면 ${manwon(EX_A.taxable)}에 대해 ${RATE}인 ${manwon(EX_A.tax)}의 세금이 발생합니다. (${OVERSEAS_STOCK_TAX.rateBreakdown}, ${TAX_BASIS_YEAR}년 기준)` },
    ...
    return (
        ...
        <CalculatorJsonLd config={config} path="/overseas-stock-tax-calculator" faqs={FAQ_ITEMS} />
            ...
            <div className="mx-auto max-w-5xl px-6 py-12 md:px-8 md:py-16">
                ...
                {/* Decision Handoff — 세액을 확인했다면 그다음은 신고 준비다 */}
                <section
                    aria-labelledby="after-calc-heading"
                    className="mt-10 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"
                >
                    <h2 id="after-calc-heading" className="text-lg font-bold text-slate-900">
                        계산했다면, 다음은 신고 준비입니다
                    </h2>
                    <p className="mt-2 text-sm leading-relaxed text-slate-600">
                        이 계산기는 예상 세액만 보여줍니다. 실제 신고에는 증권사별 양도소득 내역과
                        필요경비 확인이 필요합니다. 준비물과 순서는 아래 가이드에 정리해 두었습니다.
                    </p>
                    <Link
                        href="/guides/us-stock-tax-basic"
                        className="mt-4 inline-flex min-h-11 items-center rounded-2xl border border-slate-300 px-5 py-3 text-sm font-semibold text-slate-800 transition hover:border-slate-400"
                    >
                        해외주식 양도세 신고 준비 확인하기
                    </Link>
                </section>
                ...
                <SectionCard>
                    <Article title="해외주식 세금 계산기란?">
                        ...
                        <p>
                            국내 주식과 달리, 해외 주식은 연간 {DEDUCTION}의 기본공제를 초과하는 양도차익에
                            대해 {RATE}의 세율이 적용됩니다. 따라서 수익이 클수록 세금 계산이 중요해집니다.
                        </p>
                    </Article>
                    ...
                    <Article title="해외주식 양도소득세 계산 방법">
                        ...
                        <p>
                            예를 들어 미국 주식에서 {manwon(EX_A.gain)} 수익을 냈다면 기본공제 {DEDUCTION}을 뺀
                            {manwon(EX_A.taxable)}이 과세 대상이며, 세금은 {manwon(EX_A.tax)}입니다. 세후 수익은 {manwon(EX_A.net)}이 됩니다.
                        </p>
                        <p>
                    ...
                    <Article title="기본공제와 연간 신고 방법">
                        <p>
                            ...
                            기본공제 {DEDUCTION}은 1년에 1회 적용되며, 여러 종목에서 발생한 손익을 합산한 뒤
                            공제합니다.
                        </p>
                        <p>
            ...
            {FAQ_ITEMS.map((faq) => (
              <FaqItem key={faq.question} question={faq.question} answer={faq.answer} />
                ...
                <SeoContent
                    title="해외주식 세금 계산기"
                    intro={`해외주식에 투자할 때는 수익뿐 아니라 세금도 함께 고려해야 합니다. 양도소득세 기본공제 ${DEDUCTION}을 초과하는 수익에 대해 ${RATE}의 세율이 적용되므로, 수익 실현 전에 예상 세금을 미리 파악하는 것이 중요합니다.`}
                    body="특히 연말 절세 전략을 세울 때, 올해 남은 기본공제 한도를 파악하거나 손실 종목을 활용한 손익 통산 계획을 세울 때 이 계산기가 유용합니다."
                    formula={`세금 = (양도차익 − ${DEDUCTION} 기본공제) × ${RATE}`}
                    formulaNote={`기본공제 ${DEDUCTION}은 연간 1회 적용됩니다. ${OVERSEAS_STOCK_TAX.rateBreakdown}를 합해 ${RATE}입니다. 값의 근거와 확인 범위는 위 '계산 기준과 출처'를 확인하세요. 세법 개정 시 변동될 수 있습니다.`}
                    ...
                    example={`예시: 미국 주식을 매수 1,000만 원, 매도 1,600만 원에 처분 → 양도차익 ${manwon(EX_B.gain)}, 기본공제 ${DEDUCTION} 차감 후 과세 표준 ${manwon(EX_B.taxable)} × ${RATE} = 세금 ${manwon(EX_B.tax)}, 세후 수익 ${manwon(EX_B.net)}.`}
