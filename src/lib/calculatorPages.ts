...
export const calculatorPages: Record<string, CalculatorPageConfig> = {
    ...
    "crypto-entry-calculator": {
        ...
        metadata: {
            ...
            description:
                "진입가, 손절가, 시드, 리스크 비율을 기준으로 손절폭, 권장 배율, 주문 명목가치, 익절가를 계산하는 코인 레버리지 진입 계산기입니다.",
            keywords: [
                "코인 진입 계산기",
                "레버리지 진입 계산기",
                "비트코인 진입 계산기",
        ...
        headerDescription:
            "코인 레버리지 진입 계산기는 진입가와 손절가 사이의 손절폭을 기준으로 권장 배율, 리스크 금액, 주문 명목가치, 익절가를 계산하는 도구입니다. 레버리지 거래는 원금 손실 가능성이 매우 높은 고위험 투자이므로 신중히 사용하세요.",
        ...
        formula:
            "손절폭(%) = (진입가 - 손절가) ÷ 진입가 × 100 / 권장 배율 = 100 ÷ 손절폭(%) / 주문 명목가치 = 권장 배율 × (시드 × 리스크비율)",
        formulaDescription:
            "손절폭을 기준으로 권장 레버리지와 주문 금액을 계산하며, 시드의 일정 비율을 리스크로 설정해 실제 거래소 주문 금액(명목가치)을 확인할 수 있습니다.",
        exampleTitle: "레버리지 진입 계산 예시",
        exampleBody:
            "예를 들어 진입가가 10,000 USDT이고 손절가가 9,900 USDT라면 손절폭은 1%입니다. 이 경우 권장 배율은 100배가 되며, 시드가 100 USDT이고 리스크 비율을 5%로 잡으면 리스크 금액은 5 USDT입니다. 최종 주문 명목가치는 100배 × 5 USDT로 계산되어 500 USDT가 됩니다. 다만 권장 배율이 높아질수록 작은 가격 변동으로도 청산이 발생할 수 있으므로 실제로는 더 낮은 배율을 사용하는 것이 일반적입니다.",
        faqs: [
            {
                question: "왜 손절폭이 1%면 권장 배율이 100배가 되나요?",
                answer:
                    "이 계산기는 손절폭이 1%일 때 100배 레버리지 기준으로 자본이 모두 소진되는 구조를 기준으로 배율을 역산합니다. 실제 거래에서는 슬리피지와 수수료를 고려해 권장 배율보다 낮은 값을 사용하는 것이 안전합니다.",
            },
            {
