...
export const calculatorPages: Record<string, CalculatorPageConfig> = {
    ...
    "crypto-entry-calculator": {
        ...
        metadata: {
            ...
            description:
                "진입가, 손절가, 시드, 리스크 비율을 기준으로 손절폭, 이론 배율, 주문 명목가치, 익절가를 계산하는 코인 레버리지 진입 계산기입니다.",
            keywords: [
                "코인 진입 계산기",
                "레버리지 진입 계산기",
                "비트코인 진입 계산기",
        ...
        articleBody:
            "이 계산기는 단순 수익률 계산기가 아니라 손절폭과 시드의 일정 비율을 바탕으로 주문 명목가치를 계산하는 구조입니다. 즉, 손절가를 어디에 둘지 먼저 정한 뒤 그에 맞는 배율과 진입 금액을 역산하는 방식입니다. 레버리지 거래는 단기간에 원금 전액을 잃을 수 있는 고위험 거래이므로, 반드시 본인의 손실 감내 범위 내에서 시드와 리스크 비율을 보수적으로 설정해야 합니다.",
        formulaTitle: "레버리지 진입 계산 기본 개념",
        formula:
            "손절폭(%) = (진입가 - 손절가) ÷ 진입가 × 100 / 이론 배율 = 100 ÷ 손절폭(%) / 주문 명목가치 = 이론 배율 × (시드 × 리스크비율)",
        formulaDescription:
            "손절폭을 기준으로 이론 배율과 주문 금액을 계산하며, 시드의 일정 비율을 리스크로 설정해 실제 거래소 주문 금액(명목가치)을 확인할 수 있습니다.",
        exampleTitle: "레버리지 진입 계산 예시",
        exampleBody:
            "예를 들어 진입가가 10,000 USDT이고 손절가가 9,900 USDT라면 손절폭은 1%입니다. 이 경우 이론 배율은 100배가 되며, 시드가 100 USDT이고 리스크 비율을 5%로 잡으면 리스크 금액은 5 USDT입니다. 최종 주문 명목가치는 100배 × 5 USDT로 계산되어 500 USDT가 됩니다. 다만 이론 배율이 높아질수록 작은 가격 변동으로도 청산이 발생할 수 있으므로 실제로는 더 낮은 배율을 사용하는 것이 일반적입니다.",
        faqs: [
            {
            ...
            },
            {
                ...
                answer:
                    "전체 시드 중 몇 퍼센트를 한 번의 진입에서 리스크로 감수할지 정하는 값입니다. 기본값 5%는 시드의 5%만 위험 구간으로 두는 의미이며, 일반적으로 1~3% 수준의 보수적인 비율이 권장됩니다.",
            },
            {
