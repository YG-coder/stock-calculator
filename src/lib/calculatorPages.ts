...
export const calculatorPages: Record<string, CalculatorPageConfig> = {
    ...
    "crypto-liquidation-calculator": {
        ...
        title: "코인 청산가 계산기",
        headerDescription:
            "암호화폐 선물의 진입 가격, 레버리지, 포지션 방향과 유지증거금률을 입력해 격리마진 기준 청산가를 근사 계산하는 도구입니다.",
        articleTitle: "청산가 계산 결과는 근사값입니다",
        articleIntro:
            "비트코인, 이더리움 같은 암호화폐 선물 거래에서 격리마진 기준 청산 위험 구간을 빠르게 추정하는 도구입니다.",
        articleBody:
            "계산 결과에는 진입가, 레버리지와 사용자가 입력한 유지증거금률만 반영됩니다. 실제 거래소는 계약 종류, 포지션 규모별 위험한도, 종료 수수료, 추가 증거금과 펀딩비 등을 함께 반영하므로 표시값과 차이가 날 수 있습니다.",
        formulaTitle: "격리마진 청산가 근사식",
        formula:
            "롱: 진입가 × (1 - 1/레버리지 + 유지증거금률) / 숏: 진입가 × (1 + 1/레버리지 - 유지증거금률)",
        formulaDescription:
            "이 식은 청산 위험을 빠르게 비교하기 위한 단순 근사식입니다. 실제 청산 여부는 거래소가 정한 마크 가격과 유지증거금 조건을 기준으로 판단될 수 있습니다.",

        exampleTitle: "코인 청산가 계산 예시",
        exampleBody:
            "예를 들어 비트코인을 70,000 USDT에 10배 레버리지, 유지증거금률 0.5%로 롱 진입하면 근사 청산가는 63,350 USDT입니다. 이는 진입가보다 약 9.5% 낮은 값입니다. 실제 주문에서는 거래소 화면의 예상 청산가와 마크 가격을 다시 확인하고, 청산가에 도달하기 전 실행할 손절 기준을 별도로 정해야 합니다.",

        faqs: [
            {
                question: "청산가는 거래소마다 다른가요?",
                answer:
                    "네. 거래소마다 계약 방식, 유지증거금률, 포지션 규모별 위험한도, 종료 수수료와 청산 절차가 달라 실제 청산가가 달라질 수 있습니다.",
            },
            {
            ...
            {
                question: "마크 가격이 청산에 왜 중요한가요?",
                answer:
                    "거래소는 일시적인 체결가 움직임의 영향을 줄이기 위해 마크 가격을 청산 판단에 사용할 수 있습니다. 화면의 최근 체결가가 청산가에 닿지 않았더라도 마크 가격이 조건에 도달하면 청산될 수 있으므로 거래소 화면에서 두 가격을 구분해 확인해야 합니다.",
            },
            {
                question: "교차마진 청산가도 계산할 수 있나요?",
                answer:
                    "이 계산기의 숫자는 격리마진 기준 근사값입니다. 교차마진은 가용 잔고와 다른 포지션의 미실현 손익을 함께 반영하므로 단일 포지션 입력만으로 정확히 계산하기 어렵습니다. 거래소가 표시하는 예상 청산가와 계정 유지증거금률을 확인하세요.",
            },
            {
    ...
    "crypto-average-calculator": {
        ...
        badge: "평균 단가",
        title: "코인 물타기·불타기 계산기",
        headerDescription:
            "코인 물타기·불타기 계산기는 기존 매수와 여러 번의 추가 매수 가격·수량을 기준으로 새로운 평균 매입 단가를 계산하는 도구입니다. 하락 매수(물타기)와 상승 매수(불타기)를 모두 지원합니다.",
        ...
        formula:
            "평균 단가 = 각 매수금액의 합계 ÷ 전체 보유 수량",
        formulaDescription:
            "여러 번 매수한 경우에도 각 매수금액(매수가 × 수량)을 모두 더한 뒤 전체 수량으로 나눕니다. 기호로 쓰면 Σ(매수가 × 수량) ÷ Σ수량 입니다. 단순히 두 가격의 평균이 아니라 금액 비중과 수량을 함께 반영해야 정확합니다.",

        exampleTitle: "코인 물타기 계산 예시",
        exampleBody:
            "예를 들어 비트코인을 70,000 USDT에 0.2개 매수한 뒤, 가격이 60,000 USDT로 하락했을 때 0.1개를 추가 매수했다고 가정해보겠습니다. 이 경우 평균 단가는 단순히 두 가격의 평균이 아니라, 각 매수 금액과 수량 비중을 함께 반영해 계산해야 합니다. 이 계산기에서는 추가 매수를 여러 번 입력할 수 있고, 하락 매수(물타기)뿐 아니라 상승 매수(불타기)도 같은 방식으로 새 평균 단가와 손익분기점을 확인할 수 있습니다. 현재가를 입력하면 평가손익과 수익률도 함께 계산됩니다.",
        ...
        faqs: [
            ...
            {
                question: "현재가를 입력하면 수익률도 확인할 수 있나요?",
                answer:
                    "네. 현재가를 입력하면 새 평균 단가 기준 평가손익과 수익률이 함께 계산됩니다. 하락 매수(물타기)와 상승 매수(불타기)를 모두 지원합니다.",
            },
        ],
    ...
    "crypto-entry-calculator": {
        ...
        headerDescription:
            "코인 레버리지 진입 계산기는 진입가와 손절가 사이의 손절폭을 기준으로 이론 배율, 리스크 금액, 주문 명목가치, 익절가를 계산하는 도구입니다. 레버리지 거래는 원금 손실 가능성이 매우 높은 고위험 투자이므로 신중히 사용하세요.",
        articleTitle: "코인 레버리지 진입 계산기란?",
        articleIntro:
            "손절폭을 기준으로 몇 배 진입이 가능한지, 그리고 거래소 주문창에 얼마를 넣어야 하는지 계산하는 실전용 도구입니다.",
        articleBody:
            "이 계산기는 단순 수익률 계산기가 아니라 손절폭과 투자 원금의 일정 비율을 바탕으로 주문 명목가치를 계산하는 구조입니다. 즉, 손절가를 어디에 둘지 먼저 정한 뒤 그에 맞는 배율과 진입 금액을 역산하는 방식입니다. 레버리지 거래는 단기간에 원금 전액을 잃을 수 있는 고위험 거래이므로, 반드시 본인의 손실 감내 범위 내에서 투자 원금과 리스크 비율을 보수적으로 설정해야 합니다.",
        formulaTitle: "레버리지 진입 계산 기본 개념",
        formula:
            "손절폭(%) = (진입가 - 손절가) ÷ 진입가 × 100 / 이론 배율 = 100 ÷ 손절폭(%) / 주문 명목가치 = 이론 배율 × (투자 원금 × 리스크비율)",
        formulaDescription:
            "손절폭을 기준으로 이론 배율과 주문 금액을 계산하며, 투자 원금의 일정 비율을 리스크로 설정해 실제 거래소 주문 금액(명목가치)을 확인할 수 있습니다.",
        exampleTitle: "레버리지 진입 계산 예시",
        exampleBody:
            "예를 들어 진입가가 10,000 USDT이고 손절가가 9,900 USDT라면 손절폭은 1%입니다. 이 경우 이론 배율은 100배가 되며, 투자 원금이 100 USDT이고 리스크 비율을 5%로 잡으면 리스크 금액은 5 USDT입니다. 최종 주문 명목가치는 100배 × 5 USDT로 계산되어 500 USDT가 됩니다. 다만 이론 배율이 높아질수록 작은 가격 변동으로도 청산이 발생할 수 있으므로 실제로는 더 낮은 배율을 사용하는 것이 일반적입니다.",
        faqs: [
            {
                question: "왜 손절폭이 1%면 이론 배율이 100배가 되나요?",
                answer:
                    "이 계산기는 손절폭이 1%일 때 100배 레버리지 기준으로 자본이 모두 소진되는 구조를 기준으로 배율을 역산합니다. 실제 거래에서는 슬리피지와 수수료를 고려해 이론 배율보다 낮은 값을 사용하는 것이 안전합니다.",
            },
            {
                question: "주문 명목가치는 실제 거래소에 넣는 금액인가요?",
                answer:
                    "네, 주문 명목가치는 실제 거래소 주문창에 입력하는 금액(USDT 또는 원화)이며, 이 금액을 기준으로 포지션이 체결됩니다.",
            },
            {
                question: "리스크 비율은 무엇인가요?",
                answer:
                    "전체 투자 원금 중 몇 퍼센트를 한 번의 진입에서 리스크로 감수할지 정하는 값입니다. 예를 들어 5%를 입력하면 투자 원금의 5%를 위험 구간으로 두는 의미입니다.",
            },
            {
                question: "이 계산기는 청산가 계산기와 다른가요?",
                answer:
                    "네. 청산가 계산기는 강제 종료 가격을 계산하고, 이 계산기는 손절폭 기준으로 이론 배율과 주문 명목가치를 계산하는 도구입니다.",
            },
            {
                question: "레버리지 거래는 안전한가요?",
                answer:
                    "레버리지 거래는 원금 손실 가능성이 매우 높은 고위험 투자입니다. 가격 변동에 따라 단시간에 청산되어 원금 전액을 잃을 수 있으며, 본 계산기는 단순 참고용 도구로 투자 권유나 수익 보장을 의미하지 않습니다.",
            },
        ],
        related: [
            {
    ...
    "us-exchange-profit-calculator": {
        ...
        headerDescription:
            "미국주식 환율 반영 수익 계산기는 매수·매도 가격과 환율, 수량, 수수료를 기준으로 달러 기준 손익과 원화 기준 실제 수익을 계산하는 도구입니다.",
        articleTitle: "미국주식 환율 반영 수익 계산기란?",
        articleIntro:
            "미국주식 투자에서 환율과 수수료까지 반영해 원화 기준 실제 수익을 계산하는 도구입니다.",
        articleBody:
            "미국주식은 주가 상승뿐 아니라 매수 환율과 매도 환율의 차이도 실제 수익에 큰 영향을 미칩니다. 달러 기준으로 수익이 나더라도 환율이 불리하게 움직이면 원화 기준 수익이 줄거나 손실로 바뀔 수 있기 때문에, 환율과 수수료를 함께 계산하는 것이 중요합니다.",
        formulaTitle: "환율 반영 수익 계산 기본 개념",
        formula:
            "원화 기준 순손익 = (매도 가격 × 수량 × 매도 환율 - 매도 수수료) - (매수 가격 × 수량 × 매수 환율 + 매수 수수료)",
        formulaDescription:
            "달러 기준 손익과 원화 기준 실제 수익은 다를 수 있으며, 환율 변동이 수익률에 직접 영향을 줍니다.",
        exampleTitle: "미국주식 환율 반영 수익 계산 예시",
        exampleBody:
            "예를 들어 100달러에 10주를 매수하고 120달러에 매도했다고 가정해보겠습니다. 달러 기준으로는 200달러 수익이지만, 매수 환율이 1,350원이고 매도 환율이 1,380원이라면 원화 기준 실제 수익은 더 커질 수 있습니다. 반대로 매도 시 환율이 하락하면 달러 기준 수익이 나더라도 원화 기준 수익은 줄어들 수 있습니다.",
        faqs: [
            {
                question: "달러 기준으로 수익이면 원화로도 무조건 수익인가요?",
                answer:
                    "아니요. 매도 시 환율이 크게 하락하면 달러 기준 수익이 나더라도 원화 기준 수익이 줄어들거나 거의 없어질 수 있습니다.",
            },
            {
                question: "매수 환율과 매도 환율을 다르게 입력해야 하나요?",
                answer:
                    "네. 미국주식은 매수 시점과 매도 시점의 환율이 다르기 때문에 각각 따로 입력하는 것이 정확합니다.",
            },
            {
                question: "수수료도 꼭 반영해야 하나요?",
                answer:
                    "네. 실제 수익은 매수·매도 수수료까지 반영해야 정확하게 계산할 수 있습니다.",
            },
            {
                question: "이 계산기는 세금까지 포함하나요?",
                answer:
                    "아니요. 이 계산기는 환율과 수수료를 반영한 수익 계산용이며, 세금은 해외주식 세금 계산기를 함께 사용하는 것이 좋습니다.",
            },
        ],
        related: [
            {
    ...
    "us-dividend-calculator": {
        ...
        title: "미국주식 배당 계산기",
        headerDescription:
            "미국주식 배당 계산기는 보유 수량, 연간 주당 배당금, 평균 매수가, 환율, 원천징수세율을 기준으로 세전·세후 배당금과 배당수익률을 계산하는 도구입니다.",
        articleTitle: "미국주식 배당 계산기란?",
        articleIntro:
            "미국 배당주 투자 시 세전 배당금과 세후 실수령 배당금, 배당수익률을 함께 계산하는 도구입니다.",
        articleBody:
            "미국주식 배당은 보유 수량과 주당 배당금만으로 끝나지 않습니다. 원천징수세율과 환율을 함께 반영해야 실제로 얼마를 받는지 정확하게 파악할 수 있습니다. 특히 배당 성장주나 고배당주 투자에서는 세후 배당금과 배당수익률을 함께 보는 것이 중요합니다.",
        formulaTitle: "미국주식 배당 계산 기본 개념",
        formula:
            "세후 배당금 = (보유 수량 × 연간 주당 배당금) - 원천징수 세금 / 배당수익률 = 연간 배당금 ÷ 총 매수금액 × 100",
        formulaDescription:
        ...
        faqs: [
            {
                ...
                answer:
                    "미국주식 배당은 보통 원천징수세가 차감되기 때문에 실제로 계좌에 들어오는 세후 배당금은 세전 배당금보다 적습니다.",
            },
            {
                question: "환율도 배당금에 영향을 주나요?",
                answer:
                    "네. 미국주식 배당은 달러로 지급되므로 환율이 높을수록 원화 기준 실수령 배당금이 커질 수 있습니다.",
            },
            {
                question: "배당수익률은 어떻게 보나요?",
                answer:
                    "보통 총 매수금액 대비 연간 배당금의 비율로 계산하며, 세전 기준과 세후 기준을 함께 보는 것이 좋습니다.",
            },
            {
                question: "이 계산기는 미국 ETF 배당에도 사용할 수 있나요?",
                answer:
                    "네. 미국 ETF도 주당 분배금 또는 연간 배당금 기준 값을 입력하면 동일한 방식으로 계산할 수 있습니다.",
            },
        ],
        related: [
            {
